...
def compose_psi(x, N):
    ...
    psi = f_b + (-x[:,0]**2-x[:,1]**2+x_max**2) / x_max**2 * N.reshape(-1)
    return psi
...
model = EigenvalueProblemModel([2, 20, 20, 20, 1], Sin, compose_psi, PDE_loss, lr=8e-3, betas=[0.9, 0.999], start_eigenvalue=[1.2, 0.8])
