...
class EigenvalueProblemModel:
    ...
    def train(self, driver, drive_step, grid, perturb, epochs, minibatches=1, max_required_loss=1e-4, rtol=0.001, fraction=3, reg_param=1.):
        # Histories
        ...
        for epoch in bar:
            ...
            for n in range(minibatches):
                ...
                L_f = 1/(torch.mean(psi**2)+1e-6)

                L_reg = L_drive + L_lambda + L_f

                L_tot = reg_param * L_reg + L_PDE
            ...
            bar.set_description(f"Loss: {self.L_PDE_history[epoch*minibatches]:.4e}; En: {self.En_history[epoch*minibatches]:.4e}; c: {int(c):2d}")
